...
Ejecutar localmente: streamlit run rental.py

CREDENCIALES: Configurar en .streamlit/secrets.toml (ver secrets_and_migration.toml)
"""
...
# ─────────────────────────────────────────────
# [1] SEGURIDAD: CLIENTE SUPABASE CON st.secrets
...
def get_inventario(oficina_id=None, categoria=None, estado=None) -> pd.DataFrame:
    sb = get_supabase()
    # [2] ATRIBUTOS TÉCNICOS: nuevos campos incluidos en el SELECT
    q = sb.table("inventario").select(
        "id, categoria, talle, color, oficina_actual, estado, fecha_registro, "
        "impermeabilidad_k, contador_lavados, ultima_revision, "
        "fecha_prevista_devolucion, "          # [3] GESTIÓN DE FECHAS
        "oficinas(nombre)"
    )
    if oficina_id:
    ...
    if not res.data:
        return pd.DataFrame(columns=[
            "ID", "Categoria", "Talle", "Color", "Oficina", "Estado",
            "Fecha_Registro", "Impermeabilidad_K", "Lavados",
            "Ultima_Revision", "Devolucion_Prevista",
        ])
    ...
    for r in res.data:
        rows.append({
            ...
            "Impermeabilidad_K":   r.get("impermeabilidad_k"),
            "Lavados":             r.get("contador_lavados", 0),
            "Ultima_Revision":     r.get("ultima_revision"),
            "Devolucion_Prevista": r.get("fecha_prevista_devolucion"),
        })
    return pd.DataFrame(rows)


# [2] ATRIBUTOS TÉCNICOS: agregar_item con los 3 campos nuevos
def agregar_item(
    categoria: str,
    talle: str,
    color: str,
    oficina_id: int,
    impermeabilidad_k: float | None = None,
    ultima_revision: date | None = None,
) -> int:
    """
    Nuevos parámetros respecto a la versión anterior:
      - impermeabilidad_k  : índice de impermeabilidad en mm (columna: impermeabilidad_k)
      - ultima_revision     : fecha de la última revisión técnica  (columna: ultima_revision)
      - contador_lavados    : siempre arranca en 0 al crear el ítem (columna: contador_lavados)
    """
    sb = get_supabase()
    payload: dict = {
        "categoria":        categoria,
        ...
        "contador_lavados": 0,                # parte siempre en cero
    }
    if impermeabilidad_k is not None:
        payload["impermeabilidad_k"] = impermeabilidad_k
    if ultima_revision is not None:
        payload["ultima_revision"] = ultima_revision.strftime("%Y-%m-%d")

    res = sb.table("inventario").insert(payload).execute()
    return res.data[0]["id"] if res.data else None


# [3] + [4] GESTIÓN DE FECHAS + MANTENIMIENTO INTELIGENTE
def actualizar_estado(
    ...
    Cambios respecto a la versión anterior:
      - Acepta `fecha_prevista_devolucion` y la persiste en la BD al alquilar.
      - La limpia automáticamente cuando el ítem es devuelto.
      - [4] Incrementa `contador_lavados` cuando el ítem pasa de
        'Mantenimiento' → 'Disponible' (asume lavado en el proceso).
    """
    sb = get_supabase()
    item = sb.table("inventario").select(
        "oficina_actual, estado, contador_lavados"
    ).eq("id", item_id).single().execute()

    if not item.data:
        return False, "Ítem no encontrado."

    estado_anterior  = item.data["estado"]
    oficina_actual   = item.data["oficina_actual"]
    lavados_actuales = item.data.get("contador_lavados") or 0

    update_payload: dict = {"estado": nuevo_estado}

    # [4] MANTENIMIENTO INTELIGENTE: Mantenimiento → Disponible = +1 lavado
    if estado_anterior == "Mantenimiento" and nuevo_estado == "Disponible":
        update_payload["contador_lavados"] = lavados_actuales + 1

    # [3] GESTIÓN DE FECHAS: guardar o limpiar fecha de devolución
    if nuevo_estado == "Alquilado" and fecha_prevista_devolucion is not None:
        update_payload["fecha_prevista_devolucion"] = fecha_prevista_devolucion.strftime("%Y-%m-%d")
    elif nuevo_estado in ("Disponible", "Mantenimiento"):
        update_payload["fecha_prevista_devolucion"] = None   # limpiar al devolver

    sb.table("inventario").update(update_payload).eq("id", item_id).execute()
...
# [5] DASHBOARD: incluye conteo de devoluciones atrasadas
# ─────────────────────────────────────────────
def get_metricas() -> tuple[int, int, int, int, int]:
    """
    Retorna (total, disponibles, alquilados, mantenimiento, atrasados).
    'atrasados': ítems con estado=Alquilado cuya fecha_prevista_devolucion
    es anterior a hoy → devoluciones vencidas.
    """
    sb  = get_supabase()
    ...
    # [5] Devoluciones atrasadas: alquilados con fecha prevista < hoy
    atrasados_res = (
        sb.table("inventario")
        .select("id")
        .eq("estado", "Alquilado")
        .lt("fecha_prevista_devolucion", hoy)
        .execute()
    )
    atrasados = len(atrasados_res.data or [])

    return total, disp, alq, mant, atrasados
...
def page_stock():
    ...
    # [5] DASHBOARD: 5 columnas con la nueva métrica de atrasados
    total, disponibles, alquilados, mant, atrasados = get_metricas()
    ...
    with c5: render_metric(
        "⚠️ Dev. Atrasadas",
        atrasados,
        "requieren seguimiento" if atrasados > 0 else "al día ✓",
        "red" if atrasados > 0 else "green",
    )
...
def page_alquiler():
    ...
    with tab_alq:
        ...
        if disponibles.empty:
            ...
            # [3] GESTIÓN DE FECHAS: date_input reemplaza el text_input de notas
            fecha_dev = st.date_input(
                "📅 Fecha Prevista de Devolución",
                value=date.today(),
                min_value=date.today(),
                key="alq_fecha_dev",
                help="Fecha en que el cliente promete devolver el ítem.",
            )

            if st.button("🛷 Confirmar Alquiler"):
                ok, msg = actualizar_estado(
                    sel_id,
                    "Alquilado",
                    notas=f"Devolución prevista: {fecha_dev.strftime('%d/%m/%Y')}",
                    fecha_prevista_devolucion=fecha_dev,
                )
    ...
    with tab_dev:
        ...
        if alquilados.empty:
            ...
            # [4] Aviso contextual sobre el contador de lavados
            if nuevo == "Disponible":
                st.info("💡 Si el ítem viene de Mantenimiento, el contador de lavados se incrementará automáticamente.")

            notas = st.text_input("Notas de devolución (opcional)", key="dev_notas")
...
def page_agregar():
    ...
    with tab_item:
        ...
        if not categorias:
            ...
            # [2] ATRIBUTOS TÉCNICOS: campos nuevos en el formulario
            st.markdown("---")
            st.markdown("**⚙️ Atributos Técnicos** *(opcionales)*")
            c3, c4 = st.columns(2)
            with c3:
                impermeabilidad = st.number_input(
                    "🌧️ Impermeabilidad (K)",
                    min_value=0, max_value=30000, value=0, step=500,
                    help="Columna de agua en mm. 0 = no aplica / sin dato.",
                    key="item_impermeabilidad",
                )
            with c4:
                ultima_rev = st.date_input(
                    "🔧 Última Revisión",
                    value=None,
                    help="Fecha de la última revisión técnica del ítem.",
                    key="item_ultima_rev",
                )

            st.caption("💡 El contador de lavados arranca en 0 y se incrementa automáticamente cuando el ítem pasa de Mantenimiento → Disponible.")

            if st.button("➕ Agregar Ítem"):
                if not talle.strip() or not color.strip():
                    st.error("Completá el talle y el color.")
                else:
                    item_id = agregar_item(
                        cat, talle.strip(), color.strip(), ofi_map[ofi_sel],
                        impermeabilidad_k=impermeabilidad if impermeabilidad > 0 else None,
                        ultima_revision=ultima_rev,
                    )
                    st.success(
                        f"✅ Ítem #{item_id} agregado — {cat} {talle.upper()} {color.capitalize()} en {ofi_sel}."
                        + (f" Impermeabilidad: {impermeabilidad}K." if impermeabilidad > 0 else "")
                    )
                    st.rerun()
    ...
    with tab_ofi:
        ...
        st.dataframe(
            get_oficinas().rename(columns={"id": "ID", "nombre": "Nombre", "ciudad": "Ciudad"}),
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        try:
            total, disp, alq, mant, atrasados = get_metricas()   # [5] 5 valores
            ...
            if atrasados > 0:
                st.markdown(f"⚠️ Atrasadas: **{atrasados}**")   # [5]
        except Exception:
